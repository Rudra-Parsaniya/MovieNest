        ...
        {showActions && (
          <div className="absolute top-2 right-2 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
            <button
              ...
              className="flex items-center justify-center w-9 h-9 rounded-full bg-gray-800 hover:bg-blue-600 text-white shadow transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400"
              title="Add to Watchlist"
              disabled={addedToWatchlist}
            >
              {addedToWatchlist ? <Check className="w-5 h-5 text-green-400" /> : <Plus className="w-5 h-5" />}
            </button>
            <button
              onClick={handleAddToFavorites}
              className="flex items-center justify-center w-9 h-9 rounded-full bg-gray-800 hover:bg-yellow-600 text-white shadow transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-400"
              title="Add to Favorites"
              disabled={addedToFavorites}
            >
              {addedToFavorites ? <Check className="w-5 h-5 text-green-400" /> : <Star className="w-5 h-5" />}
            </button>
          </div>
        )}
        {/* Rating Badge */}
